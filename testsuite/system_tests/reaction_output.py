...
def assertCountsPositive(fname, header=None):
    """ Check value in the output file. All columns should be positive, while
    time column is discarded. 'header' specifies the header, which must be
    present if header is given, and must not be present if header is not given.

    Example usage:

    assertCountsPositive("output.txt")

        Check a count file from a run with a timestep of 1e-6 that ran for 100
        iterations and had no header, as in the following REACTION_DATA_OUTPUT:
...
def assertCountsPositiveOrZero(fname, header=None):
    """Check value in the output file. All columns should be positive or zero.
    ...
    assertCountsPositiveOrZero("output.txt")
    ...
    for row in range(start_row, len(lines)):
        ...
        # Compare each datum (time column is discarded)
        for col in range(1, len(file_row)):
            assert (file_row[col] >= 0), "In reaction output file '%s', data row %d, column %d value %.15g is negative." % (fname, row, col, file_row[col])


class RequireCountsPositiveOrZero:
    def __init__(self, name, header=None):
        self.name = name
        self.args = {}
        if header is not None:
    ...
    def check(self):
        assertCountsPositiveOrZero(self.name, **self.args)
...
def assertHitsCrossRelations(fname, header=None):
    ...
    for row in range(0, len(lines)):
        ...
        if(row > 1):
            assert (file_row[1] > 0), "In reaction output file '%s', data row %d, column 1 value %d is zero." % (fname, row, file_row[1])
            assert (file_row[4] > 0), "In reaction output file '%s', data row %d, column 4 value %d is zero." % (fname, row, file_row[4])
            assert (file_row[5] > 0), "In reaction output file '%s', data row %d, column 5 value %d is zero." % (fname, row, file_row[5])
            assert (file_row[6] > 0), "In reaction output file '%s', data row %d, column 6 value %d is zero." % (fname, row, file_row[6])
...
def assertCountConstraints(fname, constraints=None, totals=None, min_time=None, max_time=None, header=None, num_vals=None, minimums=None, maximums=None):
    ...
    try:
        ...
        if constraints is not None or totals is not None:
            got_vals = 0
            ...
            for line in file:
                ...
                if numpy is not None:
                    if m_constraint is not None:
                        ...
                        assert (m_result == 0).all(), "In reaction output file '%s', at time %s, a constraint is not satisfied" % (fname, cols[0])
                else:
...
def assertValidTriggerOutput(fname, data_cols, exact_time=True, header=None, event_titles=None, itertime=1e-6, xrange=None, yrange=None, zrange=None):
    """ Give an error if the specified trigger file is invalid, according to
    ...
    assertValidTriggerOutput("triggers.txt", 2, True, xrange=(-1, 1), yrange=(-1, 1), zrange=(-1, 1))
        ...
        Check a molecule count trigger file with an exact time column, assuming
        the default time step of 1e-6, and assuming that the molecule triggers
        are restricted to a 2x2x2 micron box centered at the origin.  All
        events should be unlabelled.
