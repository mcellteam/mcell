...
def uniq(iterable):
    """Return generator object consisting from the uniq items from the
    'iterable'. 'iterable' should be presorted.
...
def find_all_tests(path, name='', create_all=False):
    """Return a dictionary of TestSet objects. Searches recursively the
    supplied directory tree for the names/descriptions of the tests on the
    directory level.
...
def print_tests(t, indent='    '):
    """Print out the dictionary of tests names/descriptions on the directory
    level (aggregated).  Used when running with option '-list or -l'
...
def include_test(all_tests, rt, inc):
    """Added test names specified in 'inc' to the list of tests names
    collection 'rt'
...
def append_expanded(t, rt, prefix):
    """Recursively adds names of the test collections held in the
    TestSet.children to the list of test collections names 'rt'
...
def expand_test(all_tests, rt, exp, prefix=''):
    """Recursively expands the list 'rt' to include test collections on the
    levels below the names/paths as specified in 'rt'
...
def exclude_test(all_tests, rt, exc):
    """Remove the aggregate test name specified in 'exc' from the list of
    aggregate test names 'rt'
...
def add_to_test_suite(suite, all_tests, r):
    """Recursively add to the test suite aggregate tests described in the path
    name 'r'
