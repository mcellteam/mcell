  ...
  for (const MolOrRxnCountTerm& term: item.terms) {
    ...
    if (term.is_mol_count()) {
      ...
      if (term.type == CountType::EnclosedInWorld) {
        // count the molecule
        count_items[item_index].inc_or_dec(term.sign_in_expression, num_matches);
      }
  ...
  for (const MolOrRxnCountTerm& term: item.terms) {
    ...
    if (term.is_rxn_count() && term.rxn_rule_id == rxn->id) {
      ...
      if (term.type == CountType::RxnCountInWorld) {
        ...
        count_items[item_index].inc_or_dec(
        ...
        for (const auto it: counts_in_objects) {
          if (it.second != 0) {
            ...
            if (enclosing_volumes.contained_in_objects.count(term.geometry_object_id) != 0) {
              count_items[item_index].inc_or_dec(term.sign_in_expression, it.second);
            }
          }
        }
      }
...
void MolOrRxnCountEvent::compute_counts(std::vector<CountItem>& count_items) {
  ...
  count_items.resize(specific_mol_rxn_count_items.size());

  // initialize new count items
  for (uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
  ...
  for (Partition& p: partitions) {
    ...
    if (count_mols) {
      for (const Molecule& m: p.get_molecules()) {
        ...
        for (uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
          compute_mol_count_item(
              p, specific_mol_rxn_count_items[i], m, i,
              count_items
          );
        }
      } // for molecules
    ...
    if (count_rxns) {
      for (const BNG::RxnRule* rxn: world->get_all_rxns().get_rxn_rules_vector()) {
        ...
        // for each counting info
        for (uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
          compute_rxn_count_item(
              p, specific_mol_rxn_count_items[i], rxn, i,
              count_items
          );
        }
      } // for rxns
  ...
  for (uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
    ...
    count_items[i].value *= specific_mol_rxn_count_items[i].multiplier;
  }
}
...
void MolOrRxnCountEvent::step() {
  ...
  for (uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
    world->get_count_buffer(specific_mol_rxn_count_items[i].buffer_id).add(count_items[i]);
  }
}
...
void MolOrRxnCountEvent::compute_count_species_info(const species_id_t species_id) {
  ...
  // for each counting info
  for (MolOrRxnCountItem& count_item: specific_mol_rxn_count_items) {
...
void MolOrRxnCountEvent::dump(const std::string ind) const {
  ...
  for(uint i = 0; i < specific_mol_rxn_count_items.size(); i++) {
    cout << ind2 << i << "\n";
    specific_mol_rxn_count_items[i].dump(ind4);
  }
...
void MolOrRxnCountEvent::to_data_model(Json::Value& mcell_node) const {
  ...
  if (specific_mol_rxn_count_items.empty()) {
    return;
  }
  ...
  for (const MolOrRxnCountItem& info: specific_mol_rxn_count_items) {
