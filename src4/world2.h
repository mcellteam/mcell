...
public:
  ...
  color_id_t find_or_add_color(rgba_t rgba);
  rgba_t get_color(color_id_t) const;
...
public:
  ...
  // color IDs used for walls and geometry objects, unified for faster geometry export into
  // data model or other representations, value is in rgba format
  std::map<color_id_t, rgba_t> color_id_to_rgba;
  std::map<rgba_t, color_id_t> color_rgba_to_id;
