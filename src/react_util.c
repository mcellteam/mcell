 ...
 ************************************************************************/
double
compute_pb_factor(struct volume *world, struct rxn *rx, 
    int max_num_surf_products)
{
  ...
  if (rx->n_reactants==1)
  {
    ...
    if (max_num_surf_products > 0) world->create_shared_walls_info_flag = 1;
      ...
      ((rx->n_reactants == 3) && (num_surfaces == 1)))
  {
          ...
          && (num_surfaces < 2))
    {
      ...
      {
        pb_factor = world->time_unit*world->grid_density/3; /* 3 neighbors */
      }
      else
      {
        pb_factor = world->time_unit*world->grid_density/6; /* 2 molecules, 3 neighbors each */
      }
    }
...

    {
      ...
      else pb_factor = 1.0e11*world->grid_density/(2.0*N_AV)*sqrt(MY_PI * t_step / D_tot);
  ...
  else if ((rx->n_reactants == 2) && (num_vol_reactants == 2))
  {
    ...
    {
      eff_vel = (eff_vel_a + eff_vel_b) * world->length_unit / world->time_unit;   /* Units=um/sec */
      pb_factor = 1.0 / (2.0 * sqrt(MY_PI) * world->rx_radius_3d * world->rx_radius_3d * eff_vel);
  ...
  else if ((rx->n_reactants == 3) && (num_vol_reactants == 3))
  {
    ...
    if (eff_dif_a + eff_dif_b + eff_dif_c > 0)
    {
      ...
      pb_factor = 1.0 / (6.0 * (MY_PI) * world->rx_radius_3d * world->rx_radius_3d * (MY_PI) * world->rx_radius_3d * world->rx_radius_3d * eff_dif);
          ...
          (num_surf_reactants == 1))
  {
    ...
    if ((eff_dif_1 + eff_dif_2) > 0)
    {
      ...
      pb_factor = 2.0 * world->grid_density / (3.0 * (MY_PI) * world->rx_radius_3d * world->rx_radius_3d * eff_dif);
      pb_factor *= 1.0e30 / (N_AV*N_AV);                                               /* Convert (L/mol)^2/s to (um^3/number)^2/s */
    }
    else pb_factor = 0.0;  /* No rxn possible */
          ...
          (num_surf_reactants == 2))
  {
    ...
    if (eff_vel > 0)
    {
      eff_vel = eff_vel * world->length_unit / world->time_unit;   /* Units=um/sec */
      pb_factor = (sqrt(MY_PI) * world->grid_density * world->grid_density)/ (6.0 * eff_vel);
  ...
  else if ((rx->n_reactants == 3) && (num_surf_reactants == 3))
  {
    ...
    {
      /* basic case */
      pb_factor = (world->grid_density * world->grid_density * world->time_unit) / 6.0;
    }
    else if (num_active_reactants == 2)
    {
      pb_factor = (world->grid_density * world->grid_density * world->time_unit) / 4.0;
    }
    else if (num_active_reactants == 1)
    {
      pb_factor = (world->grid_density * world->grid_density * world->time_unit) / 2.0;
    }
