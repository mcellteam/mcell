...
int equivalent_geometry(struct pathway *p1,struct pathway *p2,int n)
{
  ...
  /* what we call below "surface" as a reactant3 may now also be a molecule */
  
  short o11,o12,o13,o21,o22,o23; /* orientations of individual reactants */
  /* flags for 3-reactant reactions signaling whether molecules orientations are     parallel one another and molecule and surface orientations are parallel 
     one another */
...
struct rxn * split_reaction(struct rxn *rx, struct mdlparse_vars *mpvp)
{
  ...
  struct rxn  *curr_rxn_ptr = NULL,  *head = NULL, *end = NULL, *rxn_equi_node = NULL, *temp_rxn = NULL;
   ...
   if(head->pathway_head->next == NULL){ 
    ...
    /* extract special reactions pathways (TRANSP,REFLEC and ABSORP) into 
       separate links if such are present */
       for(curr_ptr = head->pathway_head->next, prev_ptr = head->pathway_head; curr_ptr != NULL; prev_ptr = curr_ptr, curr_ptr = curr_ptr->next)
           {

           /* since SURFACE_CLASS is defined before DEFINE_REACTIONS
              it means that in the "pathway_head" list special_reactions
              are always after regular_reactions */
          if((curr_ptr->flags & PATHW_TRANSP) || (curr_ptr->flags & PATHW_REFLEC) || (curr_ptr->flags & PATHW_ABSORP)){ 
...
void check_reaction_for_duplicate_pathways(struct pathway **head, struct mdlparse_vars *mpvp)
{
   ...
   struct pathway *current, *next, *prev, *temp;
  ...
  /* extract  pathways with "prod_signature" field equal to NULL 
     into "null_result" list */
      ...
      /* remove nodes with "prod_signature = NULL" 
     ...
     /* now we know that "head" has not "prod_signature = NULL" field */
     /* extract nodes with "prod_signature = NULL" from inside the list */
     if(*head != NULL)
     {
        for(current = (*head)->next, prev = *head ; current != NULL; prev = current, current = current->next){
   ...
   if(current != NULL)
   {
    ...
    if(null_result == NULL){
       ...
       *head = null_result;
         ...
         /* combine the lists */
       for(current = result; current->next != NULL; current = current->next){}
       current->next = null_result;
       *head = result;
    }
	...
	/* reaction rates based on the type of reaction. */
	  ...
	  if (eff_dif_a + eff_dif_b + eff_dif_c > 0)
	  {
	   ...
	   pb_factor = (mpvp->vol->grid_density * mpvp->vol->grid_density * mpvp->vol->time_unit) / 6.0;
        ...
        }  /* end calculating of the basic reaction rate */
...
int invert_current_reaction_pathway(struct mdlparse_vars *mpvp)
{
  ...
  for (nprods=0,prodp=mpvp->pathp->product_head ; prodp!=NULL ; prodp=prodp->next)
  {
    ...
    if ((prodp->prod->flags&ON_GRID)!=0) mpvp->num_surf_products++;
  ...
  if (all_3d)
  {
    ...
    if (mpvp->pathp->reactant3!=NULL && (mpvp->pathp->reactant3->flags&NOT_FREE)!=0) all_3d = 0;
  ...
  if ((mpvp->pathp->reactant3!=NULL) && ((mpvp->pathp->reactant3->flags & IS_SURFACE) == 0))
  {
