...
int delayed_count_init(delayed_count_buffer_t *buf, int init_size) {
...
int delayed_count_add(delayed_count_buffer_t *buf, int *target, int amount) {
  ...
  /* FIXME: I have no clue what the reasoning being this "folding" is - MARKUS */
  ...
  int value = (int) (intptr_t) pointer_hash_lookup(&buf->counts, target, hash);
  value += amount;
  return pointer_hash_add(&buf->counts, (void const *) target, hash,
      (void *) (intptr_t) value);
}
...
static double *_delayed_count_new_double(delayed_count_buffer_t *buf) {
  ...
  if (buf->fill == buf->limit) {
    ...
    double *new_buf = CHECKED_MALLOC_ARRAY(double, len, "delayed count buffer");
    memcpy(new_buf, buf->buffer, fill * sizeof(double));
...
int delayed_count_add_double(delayed_count_buffer_t *buf, double *target,
  ...
  double *pvalue = (double *) pointer_hash_lookup(&buf->counts_dbl, target, hash);
  if (pvalue == NULL) {
    pvalue = _delayed_count_new_double(buf);
    if (pointer_hash_add(& buf->counts_dbl, (void const *) target, hash,
      (void *) pvalue)) {
       return 1;
    }
  } else {
...
void delayed_count_play(delayed_count_buffer_t *buf) {
  ...
  for (int i=0; i<table_size && num_items_remaining > 0; ++i)
  {
    if (buf->counts_dbl.keys[i] == NULL || buf->counts_dbl.values[i] == NULL) {
        continue;
    }
