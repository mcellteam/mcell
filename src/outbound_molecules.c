...
static transmitted_molecules_t *transmitted_molecules_add(
    transmitted_molecules_t *head, struct volume_molecule *mol,
    struct species *props, struct subvolume *target, struct vector3 *disp,
    double t_remain) {
  if (head == NULL || head->fill == MOLECULE_QUEUE_LENGTH) {
    transmitted_molecules_t *new_mqueue = CHECKED_MALLOC_STRUCT(
        transmitted_molecules_t, "outbound molecule queue page");
...
void outbound_molecules_add_release(outbound_molecules_t *queue,
  ...
  delayed_release_t *release =
      CHECKED_MALLOC_STRUCT(delayed_release_t, "delayed release");
...
void outbound_molecules_play(struct volume *world,
  ...
  while (queue->release_queue != NULL) {
    ...
    for (struct magic_list *incantation = cur->incantation; incantation != NULL;
      ...
      if (release_molecules(world, &cur->event)) {
        mcell_error(
            "Failed to perform reaction-triggered release from site '%s'.",
  ...
  while (queue->molecule_queue != NULL) {
    ...
    for (int i = 0; i < cur->fill; ++i) {
      ...
      struct volume_molecule *new_m = (struct volume_molecule *)CHECKED_MEM_GET(
          stg->mol, "volume molecule");
      * new_m = * (cur->molecules[i].molecule);
      ...
      stg->inbound = transmitted_molecules_add(
          stg->inbound, new_m, new_m->properties, new_m->subvol,
          &cur->molecules[i].disp_remainder, cur->molecules[i].time_remainder);
