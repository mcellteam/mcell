...
int create_grid(struct volume *world, struct wall *w,
    struct subvolume *guess);


void grid_neighbors(struct volume *world, struct surface_grid *grid,
...
int grid_release_check(struct release_region_data *rrd,int obj_n, int wall_n,
    int grid_n, struct release_evaluator *expr);
...
int add_more_tile_neighbors_to_list_fast(struct tile_neighbor **head, 
    struct surface_grid *orig_grid, int orig_strip, int orig_stripe, 
    int orig_flip, struct vector3 *start, struct vector3 *end, 
    int edge_index, struct surface_grid *new_grid);

int is_neighbor_tile(struct surface_grid *orig_grid, int orig_idx, 
    struct surface_grid *new_grid, int new_idx);

int get_tile_neighbor_from_list_of_vacant_neighbors(
    struct tile_neighbor *head, int index, struct surface_grid **grid, 
    int *idx);
...
void grid_all_neighbors_across_walls_through_edges(struct volume *world,
    struct grid_molecule *g, struct surface_grid *grid, int idx, 
    int create_grid_flag, int search_for_reactant, 
    struct tile_neighbor **tile_nbr_head, int *list_length);

void grid_all_neighbors_across_walls_through_vertices(struct volume *world,
    struct grid_molecule *g, struct wall_list *wall_nbr_head, 
    struct surface_grid *grid, int create_grid_flag, int search_for_reactant, 
    struct tile_neighbor **tile_nbr_head, int *list_length); 
