...
struct wall* ray_trace_2d(struct grid_molecule *g,struct vector2 *disp,struct vector2 *pos, int *kill_me, struct rxn **rxp, struct hit_data **hd_info)
{
  ...
  while (1) /* Will break out with return or break when we're done traversing walls */
  {
    ...
    {
       if(is_wall_edge_region_border(this_wall, this_edge)) this_wall_edge_region_border = 1;
    ...
    if (target_wall != NULL)
    {
      if(g->properties->flags & CAN_REGION_BORDER)
      {
        /* We hit the edge - check for the reflection/absorption from the 
         ...
         if(is_wall_edge_region_border(target_wall, target_wall->edges[target_edge_ind])) target_wall_edge_region_border = 1; 

         if(is_wall_edge_restricted_region_border(target_wall, target_wall->edges[target_edge_ind], g))
         { 
            reflect_now = 0;
            absorb_now = 0; 
...
#define CLEAN_AND_RETURN(x) do {                                          \
  ...
  do
  {
	...
	if ((smash->what&COLLIDE_MOL)!=0)
          mcell_internal_error("Detected a mol-mol collision outside of the 0.0...1.0 time window.  Iteration %"PRId64", time of collision %.8e, mol1=%s, mol2=%s",
                               world->it_time,
                               smash->t,
    ...
    for (smash = shead2; smash != NULL; smash = smash->next)
    {
      if (smash->t >= 1.0 || smash->t < 0.0)
      {
        if ((smash->what & (COLLIDE_MOL | COLLIDE_MOL_MOL | COLLIDE_MOL_GRID)) != 0)
          mcell_internal_error("Detected a mol-mol[-*] collision outside of the 0.0...1.0 time window.  Iteration %"PRId64", time of collision %.8e",
                               world->it_time,
                               smash->t);
