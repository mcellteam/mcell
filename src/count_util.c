          ...
          if (rl->reg->flags & sp->flags &
            ...
            if (crossed) {
              if (direction == 1) {
                if (hit_count->counter_type & TRIG_COUNTER) {
                  ...
                  if (rl->reg->flags & sp->flags & COUNT_HITS) {
                    UPDATE_COUNT_DBL(world, hit_count->data.move.front_hits, 1);
                    UPDATE_COUNT_DBL(world, hit_count->data.move.front_to_back, 1);
                  }
                  if (rl->reg->flags & sp->flags & COUNT_CONTENTS) {
                    UPDATE_COUNT(world, hit_count->data.move.n_enclosed, 1);
                  }
                }
              } else {
                if (hit_count->counter_type & TRIG_COUNTER) {
                  ...
                  if (rl->reg->flags & sp->flags & COUNT_HITS) {
                    UPDATE_COUNT_DBL(world, hit_count->data.move.back_hits, 1);
                    UPDATE_COUNT_DBL(world, hit_count->data.move.back_to_front, 1);
                  }
                  if (rl->reg->flags & sp->flags & COUNT_CONTENTS) {
                    UPDATE_COUNT(world, hit_count->data.move.n_enclosed, -1);
                  }
            ...
            if ((count_hits && rl->reg->area != 0.0) &&
              ...
              if ((hit_count->counter_type & TRIG_COUNTER) == 0) {
                UPDATE_COUNT_DBL(world, hit_count->data.move.scaled_hits,
          ...
          if (c->counter_type & TRIG_COUNTER) {
            ...
            if (am->properties->flags & ON_GRID) {
              if ((c->orientation == ORIENT_NOT_SET) ||
                ...
                UPDATE_COUNT(world, c->data.move.n_at, n);
              }
            } else {
              UPDATE_COUNT(world, c->data.move.n_at, n);
            }
          } else {
            UPDATE_COUNT_DBL(world, c->data.rx.n_rxn_at, n);
          }
        }
      }
    }
          ...
          if (c->target == target && c->reg_type == rl->reg &&
            ...
            if (c->counter_type & TRIG_COUNTER) {
              ...
              if (am->properties->flags & ON_GRID) {
                if ((c->orientation == ORIENT_NOT_SET) ||
                    (c->orientation == orient) || (c->orientation == 0)) {
                  UPDATE_COUNT(world, c->data.move.n_enclosed, n * pos_or_neg);
                }
              } else {
                UPDATE_COUNT(world, c->data.move.n_enclosed, n * pos_or_neg);
              }
            } else {
              UPDATE_COUNT_DBL(world, c->data.rx.n_rxn_enclosed, n * pos_or_neg);
            }
          }
        }
      }
...
void count_moved_surface_mol(struct volume *world, struct surface_molecule *sm,
  ...
  if (sm->properties->flags & COUNT_ENCLOSED) { /* Have to raytrace */
      ...
      for (wl = sv->wall_head; wl != NULL; wl = wl->next) {
        ...
        if (j != COLLIDE_MISS && (hit.x - target.x) * delta.x +
          ...
          for (rl = wl->this_wall->counting_regions; rl != NULL;
            ...
            if (j == COLLIDE_FRONT) {
              prl = (struct region_list *)CHECKED_MEM_GET(stor->regl,
                                                          "region list entry");
              prl->reg = rl->reg;
              prl->next = pos_regs;
              pos_regs = prl;
            } else if (j == COLLIDE_BACK) {
              nrl = (struct region_list *)CHECKED_MEM_GET(stor->regl,
                                                          "region list entry");
              nrl->reg = rl->reg;
              nrl->next = neg_regs;
              neg_regs = nrl;
    ...
    if (pos_regs != NULL) {
      pos_regs =
          (struct region_list *)void_list_sort((struct void_list *)pos_regs);
    }
    if (neg_regs != NULL) {
      neg_regs =
          (struct region_list *)void_list_sort((struct void_list *)neg_regs);
    }
          ...
          if (c->target == sm->properties && c->reg_type == rl->reg &&
            ...
            if (c->counter_type & TRIG_COUNTER) {
              ...
              UPDATE_TRIGGER(world, c, n, where, REPORT_CONTENTS|REPORT_ENCLOSED|REPORT_TRIGGER);
                       ...
                       (c->orientation == 0)) {
              UPDATE_COUNT(world, c->data.move.n_enclosed, n);
            }
          }
        }
      }
...
*************************************************************************/
static int
get_counting_regions_for_waypoint(struct subvolume *my_sv, struct waypoint *wp,
                                  struct region_list **p_all_regs,
                                  struct region_list **p_all_antiregs,
  ...
  for (rl = wp->regions; rl != NULL; rl = rl->next) {
    ...
    struct region_list *nrl = (struct region_list *)CHECKED_MEM_GET(
        my_sv->local_storage->regl, "region list entry");
    nrl->reg = rl->reg;
    nrl->next = *p_all_regs;
    *p_all_regs = nrl;
  }
...
antiregions
   ...
   Out: None
*************************************************************************/
static int get_counting_regions_for_point(
    struct volume *world, struct subvolume *my_sv, struct waypoint *wp,
  ...
  *p_all_antiregs = NULL;
  ...
  get_counting_regions_for_waypoint(my_sv, wp, &all_regs, &all_antiregs,
                                    region_hash);
  ...
  for (sv = my_sv; sv != NULL;
    ...
    for (wl = sv->wall_head; wl != NULL; wl = wl->next) {
       ...
       * plausible.
       */
      if ((hit.x - loc->x) * delta.x + (hit.y - loc->y) * delta.y +
              (hit.z - loc->z) * delta.z >=
          0)
        continue;
...
static void scan_complex_update_table(struct species **relatives,
                                      short *orients, int num_relatives,
                                      struct complex_counter *counter,
                                      int rules_start, int rules_end, int n) {
  ...
  for (int rule_index = rules_start; rule_index < rules_end; ++rule_index,
           nptr += num_relatives, iptr += num_relatives,
           optr += num_relatives) {
    ...
    for (neighbor_index = 0; neighbor_index < num_relatives; ++neighbor_index) {
      ...
      if (nptr[neighbor_index] == NULL) {
        ...
        /* If optr[neighbor_index] is 0, we don't care about orientation.
         * Continue. */
      ...
      if (iptr[neighbor_index]) {
        ...
        if (orients == NULL ||
            optr[neighbor_index] * orients[neighbor_index] >= 0)
          break;
      } else {
        if (nptr[neighbor_index] != relatives[neighbor_index])
          break;
...
count_complex_for_single_region:
   ...
   Out: None
*************************************************************************/
static void count_complex_for_single_region(
    struct complex_counter *c, struct complex_species *spec, int this_orient,
    struct species **before, short *orient_before, struct species **after,
    short *orient_after, int *update_subunit, int amount) {
  /* Now, add all relevant subunit updates to the counters */
  struct species *relatives_before[spec->num_relations + 1],
      *relatives_after[spec->num_relations + 1];
  short relatives_orient_before[spec->num_relations + 1],
      relatives_orient_after[spec->num_relations + 1];
  ...
  for (int subunit_index = 0; subunit_index < spec->num_subunits;
    ...
    for (relation_idx = 0; relation_idx < spec->num_relations; ++relation_idx) {
      ...
      if (orient_before)
        relatives_orient_before[relation_idx + offset] =
            orient_before[target_index];
      if (orient_after)
        relatives_orient_after[relation_idx + offset] =
            orient_after[target_index];
    }

    for (struct complex_counter *cCur = c; cCur != NULL; cCur = cCur->next) {
      if (this_orient != 0 && c->this_orient != 0 &&
          c->this_orient != this_orient)
        continue;

      /* Remove "before" counts */
      if (before[subunit_index] != NULL) {
        int *before_indices = (int *)pointer_hash_lookup(
            &c->subunit_to_rules_range, before[subunit_index],
            before[subunit_index]->hashval);
        if (before_indices != NULL)
          scan_complex_update_table(
              relatives_before, orient_before ? relatives_orient_before : NULL,
              spec->num_relations + offset, c, before_indices[0],
              before_indices[1], -amount);
      }

      /* Add "after" counts */
      if (after[subunit_index] != NULL) {
        int *after_indices = (int *)pointer_hash_lookup(
            &c->subunit_to_rules_range, after[subunit_index],
            after[subunit_index]->hashval);
        if (after_indices != NULL)
          scan_complex_update_table(
              relatives_after, orient_before ? relatives_orient_after : NULL,
              spec->num_relations + offset, c, after_indices[0],
              after_indices[1], amount);
      }
    }
  }
}
...
static void count_complex_new_for_single_region(struct complex_counter *c,
  ...
  for (int subunit_index = 0; subunit_index < spec->num_subunits;
    ...
    for (int relation_idx = 0; relation_idx < spec->num_relations;
         ++relation_idx) {
      int target_index = spec->relations[relation_idx].target[subunit_index];
      relatives[relation_idx + offset] = specs[target_index];
      if (orients)
        relatives_orient[relation_idx + offset] = orients[target_index];
    }
    ...
    for (struct complex_counter *cCur = c; cCur != NULL; cCur = cCur->next) {
      ...
      /* Add counts */
      indices = (int *)pointer_hash_lookup(&c->subunit_to_rules_range,
                                           specs[subunit_index],
                                           specs[subunit_index]->hashval);
      if (indices != NULL)
        scan_complex_update_table(relatives, relatives_orient,
                                  spec->num_relations + offset, c, indices[0],
                                  indices[1], amount);
    }
  }
}
...
int count_complex(struct volume *world, struct volume_molecule *cmplex,
  ...
  macro_count_inverse_related_subunits(spec, update_subunit,
                                       replaced_subunit_idx);
  ...
  before[replaced_subunit_idx] =
      replaced_subunit ? replaced_subunit->properties : NULL;

  /* Do any relevant counting for WORLD */
  count_complex_for_single_region(&spec->counters->in_world, spec, 0, before,
                                  NULL, after, NULL, update_subunit, 1);
  ...
  for (struct region_list *rl = all_regs; rl != NULL; rl = rl->next) {
    ...
    count_complex_for_single_region(c, spec, 0, before, NULL, after, NULL,
                                    update_subunit, 1);
  }
...
int count_complex_surface(struct surface_molecule *cmplex,
  ...
  before[replaced_subunit_idx] =
      replaced_subunit ? replaced_subunit->properties : NULL;
  orient_before[replaced_subunit_idx] =
      replaced_subunit ? replaced_subunit->orient : 0;

  /* Do any relevant counting for WORLD */
  count_complex_for_single_region(&spec->counters->in_world, spec,
                                  cmplex->orient, before, orient_before, after,
                                  orient_after, update_subunit, 1);
  ...
  if (my_wall != NULL && (my_wall->flags & COUNT_CONTENTS) != 0) {
    for (struct region_list *rl = my_wall->counting_regions; rl != NULL;
      ...
      count_complex_for_single_region(c, spec, cmplex->orient, before,
                                      orient_before, after, orient_after,
                                      update_subunit, 1);
    }
  }
  return 0;
...
macro_copy_count_requests_to_tables:
   ...
   In:  struct pointer_hash *requests_by_subunit - the requests, sorted by
subunit
        struct pointer_hash *subunit_to_rules_range - index giving range of
rules in table by subunit
        ...
        int *su_rules_indices - array of indices within nptr and iptr of rules
by subunit
        int *counts - array of ints which hold the actual counts during the
simulation
   Out: 0 on success, 1 on failure
*************************************************************************/
static int
macro_copy_count_requests_to_tables(struct pointer_hash *requests_by_subunit,
                                    struct pointer_hash *subunit_to_rules_range,
                                    struct species **nptr, int *iptr,
                                    signed char *optr, int num_relations,
                                    int *su_rules_indices, int *counts) {
  int table_position = 0, start_pos;
  int su_index = 0;
  for (int bin_index = 0; bin_index < requests_by_subunit->table_size;
       ++bin_index) {
    /* Skip empty bins */
    ...
    struct macro_count_request *head =
        (struct macro_count_request *)requests_by_subunit->values[bin_index];
    struct macro_count_request *mcr, *mcrnext;
    int offset = optr ? 1 : 0;
    for (mcr = head; mcr != NULL; mcr = mcrnext) {
      ...
      if (optr) {
        nptr[0] = NULL;
        iptr[0] = 0;
        if (mcr->subunit_orientation > 0)
          optr[0] = 1;
        else if (mcr->subunit_orientation < 0)
          optr[0] = -1;
        else
          optr[0] = 0;
      }
      ...
      for (msr = mcr->relation_states; msr != NULL; msr = msrnext) {
        ...
        if (optr) {
          if (msr->orient > 0)
            optr[offset + msr->relation] = 1;
          else if (msr->orient < 0)
            optr[offset + msr->relation] = -1;
          else
            optr[offset + msr->relation] = 0;
        }
        free(msr);
      }
    ...
    /* Add to the index */
    struct species *key =
        (struct species *)requests_by_subunit->keys[bin_index];
    if (pointer_hash_add(subunit_to_rules_range, key, key->hashval,
                         su_rules_indices + su_index))
      mcell_allocfailed(
          "Failed to initialize macromolecule state count rules table.");
...
macro_sort_output_requests_by_orientation:
   ...
   Out: None.  'by_orientation' array has been updated with requests separated
        by orientation.
*************************************************************************/
static void macro_sort_output_requests_by_orientation(
    struct macro_count_request *requests,
...
static void macro_destroy_counters(struct complex_species *spec) {
  ...
  if (spec->counters->in_regions) {
    for (int region_idx = 0; region_idx < spec->counters->num_region_counters;
         ++region_idx) {
      ...
      pointer_hash_destroy(
          &spec->counters->in_regions[region_idx].subunit_to_rules_range);
    }
...
*************************************************************************/
static int
macro_collect_count_requests_by_location(struct macro_count_request *requests,
...
macro_create_region_counters:
   ...
   In:  struct complex_counters *c - the counters into which to add region
counters
...
static int macro_create_region_counters(struct complex_counters *c,
  ...
  c->in_regions =
      CHECKED_MALLOC_ARRAY(struct complex_counter, num_region_counters,
                           "macromolecular complex per-region counters");
  memset(c->in_regions, 0,
         num_region_counters * sizeof(struct complex_counter));
  c->num_region_counters = num_region_counters;

  /* Now initialize each counter */
  for (int counter_index = 0; counter_index < c->num_region_counters;
       ++counter_index) {
    if (pointer_hash_init(&c->in_regions[counter_index].subunit_to_rules_range,
                          16))
      mcell_allocfailed(
          "Failed to initialize subunit->rules counter for region.");
  }
...
static int macro_convert_output_requests_for_complex(
  ...
  if (requests_by_region.num_items != 0) {
    ...
    for (int bin_index = 0; bin_index < requests_by_region.table_size;
      ...
      struct macro_count_request *my_requests =
          (struct macro_count_request *)requests_by_region.values[bin_index];
...
macro_expand_object_output:
   ...
   Out: 0 on success, 1 on failure (memory allocation only?).
        ...
        is then added up here.
*************************************************************************/
static int macro_expand_object_output(struct macro_count_request *request,
                                      struct object *obj) {
  switch (obj->object_type) {
  case META_OBJ:
#ifdef ALLOW_COUNTS_ON_METAOBJECT
#error                                                                         \
    "Support for counting macromolecules in/on a metaobject is unimplemented."
#endif
    mcell_error(
        "COUNT and TRIGGER statements on metaobject '%s' are not allowed.",
        obj->sym->name);
...
static int macro_normalize_output_request_locations(
  ...
  for (struct macro_count_request *mcr = macro_count_request_head; mcr != NULL;
       mcr = mcr->next) {
    ...
    if (!is_object_instantiated(mcr->location, root_instance)) {
      mcell_error("The object/region name '%s' in the COUNT/TRIGGER statement "
                  "is not fully referenced.\n"
                  "  This occurs when a count is requested on an object which "
    ...
    if (mcr->location->sym_type == OBJ) {
      if (macro_expand_object_output(mcr,
                                     (struct object *)mcr->location->value))
        mcell_error("Failed to expand request to count on object.");
    } else {
...
macro_collect_count_requests_by_complex(struct pointer_hash *h,
  ...
  for (mcr = head; mcr != NULL; mcr = mcrnext) {
    ...
    if (pointer_hash_add(h, c, c->base.hashval, mcr))
      mcell_allocfailed(
          "Failed to add count request to complex->counter hash.");
  }
