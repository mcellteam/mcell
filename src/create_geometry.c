...
*************************************************************************/
int
check_patch(struct subdivided_box *b,
            struct vector3 *p1,
            struct vector3 *p2,
            double egd)
{
...
*************************************************************************/
int
normalize_elements(struct region *reg,
                   int existing)
{
...
**************************************************************************/
struct region *
create_region(MCELL_STATE* state, struct object *obj_ptr, char *name)
{
...
*************************************************************************/
struct region *
make_new_region(MCELL_STATE* state, char *obj_name, char *region_last_name)
{
  char *region_name;
  ...
  if ((sym_ptr = store_sym(region_name, REG, state->reg_sym_table, NULL)) == NULL)
  {
    free(region_name);
    return NULL;
  }
