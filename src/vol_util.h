...
struct subvolume* traverse_subvol(struct subvolume *here,
    struct vector3 *point, int which, int nx_parts, int ny_parts,
    int nz_parts);
...
struct grid_molecule* place_grid_molecule(struct volume *world, 
    ...
    double t, struct subvolume **psv, struct grid_molecule **cmplx);

struct grid_molecule *insert_grid_molecule(struct volume *world, 
    struct species *s, struct vector3 *loc, short orient, 
    double search_diam, double t, struct grid_molecule **cmplx);

struct volume_molecule *insert_volume_molecule(struct volume* world,
    struct volume_molecule *m, struct volume_molecule *guess);
...
void randomize_vol_mol_position(struct volume *world, 
    struct volume_molecule *mp, struct vector3 *low_end, double size_x, 
