...
struct complex_species *new_complex_species(int num_subunits, int type) {
  ...
  specp->subunits = CHECKED_MALLOC_ARRAY(struct species *, num_subunits,
  ...
  /* Allocate array of subunit locations (relative to macromol's placement
   * point) */
  specp->rel_locations = CHECKED_MALLOC_ARRAY(
      struct vector3, num_subunits, "complex species subunit positions");
  return specp;
}
...
static double macro_lookup_rate_grid(struct complex_rate const *r,
  ...
  int subunit_idx =
      macro_subunit_index((struct abstract_molecule const *)subunit);
  ...
  for (int relation_idx = 0; relation_idx < s->num_relations; ++relation_idx) {
    subunit_types[relation_idx] =
        c[s->relations[relation_idx].target[subunit_idx] + 1]->properties;
    orientations[relation_idx] =
        c[s->relations[relation_idx].target[subunit_idx] + 1]->orient;
  }
  ...
  for (int rule_idx = 0; rule_idx < r->num_rules; ++rule_idx) {
    ...
    for (neighbor_idx = 0; neighbor_idx < s->num_relations; ++neighbor_idx) {
      if (r->neighbors[neighbor_offset + neighbor_idx] == NULL)
        ...
        if (r->neighbors[neighbor_offset + neighbor_idx] ==
            subunit_types[neighbor_idx]) {
          if (!r->orientations ||
              r->orientations[neighbor_offset + neighbor_idx] *
...
/*******************************************************************************
 macro_lookup_relation:
    ...
    In:  struct complex_species const *cs - the species whose relation to look
up
...
int macro_lookup_relation(struct complex_species *cs, char const *name) {
  for (int relation_index = 0; relation_index < cs->num_relations;
       ++relation_index) {
...
static struct wall *ray_trace_to_subunit(struct volume *world, struct wall *w,
  ...
  while (1) /* Will break out with return or break when we're done traversing
               walls */
  {
    new_wall_index =
    ...
    if (new_wall_index ==
        -1) /* We didn't hit the edge.  Stay inside this wall. */
    {
    ...
    target_wall =
        traverse_surface(this_wall, &old_pos, new_wall_index, &this_pos);
...
static int macro_place_subunits_grid(struct volume *world,
  ...
  for (int subunit_idx = 0; subunit_idx < s->num_subunits; ++subunit_idx) {
    ...
    for (int tries = 0; tries < 5 && new_wall == NULL; ++tries) {
      ...
      new_wall = ray_trace_to_subunit(world, master->grid->surface, &disp,
                                      &pos2, rgn, rrd);

      /* If we failed to place this subunit, try rotating the position very
       * slightly */
      if (new_wall == NULL) {
...
int macro_place_subunits_volume(struct volume *world,
  ...
  master->cmplx = CHECKED_MALLOC_ARRAY(
      struct volume_molecule *, (s->num_subunits + 1), "volume macromolecule");
  memset(master->cmplx, 0,
         sizeof(struct volume_molecule *) * (s->num_subunits + 1));
  ...
  for (int subunit_idx = 0; subunit_idx < s->num_subunits; ++subunit_idx) {
    ...
    new_subunit.birthday = convert_iterations_to_real_time(
        world->start_iterations, world->time_unit,
        world->current_start_real_time, master->t);
    ...
    master->cmplx[subunit_idx + 1] = guess = subunit =
        insert_volume_molecule(world, &new_subunit, guess);
    if (subunit == NULL)
      return 1;
...
macro_insert_molecule_grid_2(struct volume *world, struct species *spec,
  ...
  master->birthday = convert_iterations_to_real_time(
      world->start_iterations, world->time_unit,
      world->current_start_real_time, event_time);
...
void macro_count_inverse_related_subunits(struct complex_species *spec,
  ...
  for (int relation_index = 0; relation_index < spec->num_relations;
    ...
    if (spec->relations[relation_index].inverse) {
      int subunit_index =
          spec->relations[relation_index].inverse[target_subunit];
